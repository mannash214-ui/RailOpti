...
for (let cIdx = 0; cIdx < cleanCorridors.length; cIdx++) {
  ...
  if (corridor.length < 5) continue; // Skip short corridors
  ...
  for (let run = 0; run < runsPerCorridor; run++) {
    ...
    for (const direction of ['UP', 'DOWN']) {
      ...
      if (finalStops.length < 3) continue;
      ...
      for (let i = 0; i < finalStops.length; i++) {
        ...
        if (i === 0) {
          ...
          if (travelMinutes < 10) travelMinutes = 10; // Enforce minimum distance travel time
