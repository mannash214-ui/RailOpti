...
async function runTests() {
    ...
    await trainStop_model_1.TrainStop.deleteMany({ platform: 'TEST_PLAT' });
    await train_model_1.Train.deleteMany({ trainName: /TEST_TRAIN/ });
    await station_model_1.Station.deleteMany({ city: 'TEST_CITY' });
    console.log('[Date-Aware Planner Tests] Seeding temporary test stations...');
    const stationA = await station_model_1.Station.create({
        name: 'Test Station A',
        stationCode: 'TSA',
        city: 'TEST_CITY',
        state: 'Test State',
    ...
    const stationB = await station_model_1.Station.create({
        name: 'Test Station B',
        stationCode: 'TSB',
        city: 'TEST_CITY',
        state: 'Test State',
    ...
    const stationC = await station_model_1.Station.create({
        name: 'Test Station C',
        stationCode: 'TSC',
        city: 'TEST_CITY',
        state: 'Test State',
    ...
    // 1. Monday-only train
    const trainMon = await train_model_1.Train.create({
        trainNumber: 'TMON1',
        trainName: 'TEST_TRAIN_MON',
        trainType: types_1.TrainType.EXPRESS,
        operatingDays: [types_1.OperatingDay.MON],
        sourceStation: stationA._id,
        destinationStation: stationB._id,
        averageDelayMinutes: 5,
        cancellationProbability: 0.01,
    });
    await trainStop_model_1.TrainStop.create([
    ...
    const trainWed = await train_model_1.Train.create({
        trainNumber: 'TWED1',
        trainName: 'TEST_TRAIN_WED',
        trainType: types_1.TrainType.EXPRESS,
        operatingDays: [types_1.OperatingDay.WED],
        sourceStation: stationA._id,
        destinationStation: stationB._id,
    ...
    // 3. Weekend train
    const trainWek = await train_model_1.Train.create({
        trainNumber: 'TWEK1',
        trainName: 'TEST_TRAIN_WEK',
        trainType: types_1.TrainType.SUPERFAST,
        operatingDays: [types_1.OperatingDay.SAT, types_1.OperatingDay.SUN],
        sourceStation: stationA._id,
        destinationStation: stationB._id,
    ...
    // 4. Daily train
    const trainDly = await train_model_1.Train.create({
        trainNumber: 'TDLY1',
        trainName: 'TEST_TRAIN_DLY',
        trainType: types_1.TrainType.SUPERFAST,
        operatingDays: [types_1.OperatingDay.MON, types_1.OperatingDay.TUE, types_1.OperatingDay.WED, types_1.OperatingDay.THU, types_1.OperatingDay.FRI, types_1.OperatingDay.SAT, types_1.OperatingDay.SUN],
        sourceStation: stationA._id,
        destinationStation: stationB._id,
    ...
    const trainOvn = await train_model_1.Train.create({
        trainNumber: 'TOVN1',
        trainName: 'TEST_TRAIN_OVN',
        trainType: types_1.TrainType.RAJDHANI,
        operatingDays: [types_1.OperatingDay.WED],
        sourceStation: stationA._id,
        destinationStation: stationB._id,
        averageDelayMinutes: 0,
    ...
    const trainTrsfA = await train_model_1.Train.create({
        trainNumber: 'TTRFA',
        trainName: 'TEST_TRAIN_TRFA',
        trainType: types_1.TrainType.SHATABDI,
        operatingDays: [types_1.OperatingDay.MON, types_1.OperatingDay.TUE, types_1.OperatingDay.WED, types_1.OperatingDay.THU, types_1.OperatingDay.FRI, types_1.OperatingDay.SAT, types_1.OperatingDay.SUN],
        sourceStation: stationA._id,
        destinationStation: stationB._id,
        averageDelayMinutes: 0,
    ...
    const trainTrsfB = await train_model_1.Train.create({
        trainNumber: 'TTRFB',
    ...
    try {
        ...
        assert(err instanceof error_1.AppError && err.statusCode === 400 && err.message.includes('Past travel dates'), `Past travel date correctly rejected: ${err.message}`);
    }
    // ----------------------------------------------------
    ...
    try {
        ...
        assert(err instanceof error_1.AppError && err.statusCode === 400 && err.message.includes('Invalid calendar date'), `Invalid calendar date correctly rejected: ${err.message}`);
    }
