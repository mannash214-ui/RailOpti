...
class SearchEngine {
    ...
    costStrategy;
    constraintChecker;
    constructor(graph, config, costStrategy, constraintChecker) {
        this.graph = graph;
        this.config = config;
     ...
     */
    search(sourceStationCode, destinationStationCode, departureMinutes) {
        const distTable = new Map();
        ...
        for (const node of sourceNodes) {
            if (node.absoluteDepartureMinutes >= departureMinutes &&
        ...
        while (pq.size() > 0) {
            ...
            for (let i = 0; i < edges.length; i++) {
                const edge = edges[i];
                if (edge.type === types_1.EdgeType.TRAVEL) {
                ...
                else if (edge.type === types_1.EdgeType.TRANSFER) {
    ...
    getTableValue(map, key1, key2) {
        return map.get(key1)?.get(key2);
    }
    setTableValue(map, key1, key2, val) {
